...
async function getDbNameList(): Promise<DbResult> {
    const res: ListDatabasesResult = await mongoClient.db().admin().listDatabases();
...
export function validateDatabaseToDelete(dbRequest: DbRequest, cachedDbList = getCachedDbList()): string {
    const validationResult = validateDatabaseNameInRequestBody(dbRequest);
    if (validationResult) return validationResult;
    const name = dbRequest.body.name;
    if (!cachedDbList.some(item => item.name === name)) return `Database with the name '${name}' does not exist.`;
    return "";
}

export function validateRequestDatabaseExists(dbRequest: DbRequest, cachedDbList = getCachedDbList()): string {
